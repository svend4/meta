...
#   5) Д  — △ (треугольник: B+D1+D2)*        → h=50  (B+D1+D2) *
#   6) В  — N-образный (L+D1)*              → h=20  (L+D1) *
#   7) Б  — И-образный (L+B+R)*             → h=14  (B+L+R) *
#   8) ...                                   → TBD
#
# Столбец 2 (правый, снизу вверх — буквы 9–16):
#   ...
#   12) Р  — □ (рамка четырёх сторон)        → h=15  (T+B+L+R)
#   13) У  — □-вариант*                      → h=15  (T+B+L+R) *
...
RUSSIAN_PHONETIC: dict[str, dict] = {
    ...
    'Д': {'h': 50, 'segs': 'B+D1+D2',         'char': 'C',
          'desc': 'треугольник △',            'confidence': 'low'},
}
